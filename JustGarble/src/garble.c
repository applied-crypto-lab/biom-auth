...
int createNewWire(Wire *in, GarblingContext *garblingContext, int id) {
	...
	garblingContext->fixedWires[id] = VARIABLE_VAL_WIRE;
	return 0;
}
...
int createEmptyGarbledCircuit(GarbledCircuit *garbledCircuit, int n, int m,
	...
	garbledCircuit->n = n;
	garbledCircuit->q = q;
	garbledCircuit->qand = 0;
	garbledCircuit->qor = 0;
	garbledCircuit->qxor = 0;
	garbledCircuit->qnot = 0;
	garbledCircuit->r = r;
	int i;
	for (i = 0; i < r; i++) {
...
int startBuilding(GarbledCircuit *garbledCircuit,
	...
	for (int i = 0; i < garbledCircuit->n; i++)
		garblingContext->fixedWires[i] = INPUT_VAL_WIRE;
...
int finishBuilding(GarbledCircuit *garbledCircuit,
	...
	garbledCircuit->r = garbledContext->wireIndex;

	for (i = 0; i < garbledCircuit->r; i++) {
		if (garbledContext->fixedWires[i] == FIXED_ZERO_WIRE) {
			//printf("Wire %i is fixed 0 wire\n", i);
			garbledCircuit->wires[i].label = garbledCircuit->wires[i].label0;
		}
		if (garbledContext->fixedWires[i] == FIXED_ONE_WIRE) {
			//printf("Wire %i is fixed 1 wire\n", i);
	...
	for (i = 0; i < garbledCircuit->m; i++) {
		...
		if ((garbledContext->fixedWires[outputs[i]] != FIXED_ZERO_WIRE) && (garbledContext->fixedWires[outputs[i]] != FIXED_ONE_WIRE))
			garbledContext->fixedWires[outputs[i]] = OUTPUT_VAL_WIRE;
		garbledCircuit->outputs[i] = outputs[i];
	}

	printf("Num AND gates: %d\n", garbledCircuit->qand);
	printf("Num OR gates: %d\n", garbledCircuit->qor);
	printf("Num XOR gates: %d\n", garbledCircuit->qxor);
	printf("Num NOT gates: %d\n", garbledCircuit->qnot);
	printf("Num total gates: %d\n", garbledCircuit->qand + garbledCircuit->qor + garbledCircuit->qxor+ garbledCircuit->qnot);
...
long garbleCircuit(GarbledCircuit *garbledCircuit, InputLabels inputLabels, OutputMap outputMap) {
  ...
  seedRandom();
	...
	AES_KEY_JG KR;
	AES_set_encrypt_key_JG(&rkey, 128, &KR);
...
long garbleCircuit(GarbledCircuit *garbledCircuit, InputLabels inputLabels,
	...
	block rkey = randomBlock();
	AES_KEY_JG KR;
	AES_set_encrypt_key_JG((unsigned char *) &rkey, 128, &KR);
	const __m128i *sched2 = ((__m128i *) (KR.rd_key));
	garblingContext.R =
			xorBlocks(garbledCircuit->wires[0].label0, garbledCircuit->wires[0].label1);
	garbledCircuit->globalKey = key;
...
int createInputLabels(InputLabels inputLabels, int n) {
	...
	*((short *) (&R)) = 1;
	...
	for (i = 0; i < 2 * n; i += 2) {
                randAESBlock(((block *) temp), rand_context);

...
__m128i __current_rand_index;
AES_KEY_JG __rand_aes_key;
...
void seedRandom() {
  if (!already_initialized) {
    ...
    srand(time(NULL));
  	...
  	AES_set_encrypt_key_JG((unsigned char *) &cur_seed, 128, &__rand_aes_key);
  }
}

void JGseedRandomCust(const unsigned char *userKey) {
	__current_rand_index = zero_block();
	AES_set_encrypt_key_JG(userKey, 128, &__rand_aes_key);
}

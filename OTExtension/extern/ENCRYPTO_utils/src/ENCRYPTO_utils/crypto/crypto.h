...
public:
	...
	crypto(uint32_t statbits, uint32_t symsecbits);
	crypto(uint32_t statbits, uint32_t symsecbits, uint8_t* seed);
	~crypto();
...
private:
	...
	void init(uint32_t statbits, uint32_t symsecbits, uint8_t* seed);

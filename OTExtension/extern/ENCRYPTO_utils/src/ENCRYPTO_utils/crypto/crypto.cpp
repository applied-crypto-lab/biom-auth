...
crypto::crypto(uint32_t statbits, uint32_t symsecbits) {
	uint8_t* seed = (uint8_t*) malloc(sizeof(uint8_t) * AES_BYTES);
	gen_secure_random(seed, AES_BYTES);

	init(statbits, symsecbits, seed);
	free(seed);
}

crypto::crypto(uint32_t statbits, uint32_t symsecbits, uint8_t* seed) {
	init(statbits, symsecbits, seed);
}
...
void crypto::init(uint32_t statbits, uint32_t symsecbits, uint8_t* seed) {
	if (statbits == 40)
		secparam = get_sec_lvl(symsecbits);
	else {
		secparam = {statbits, symsecbits, 0};
		if (secparam.symbits <= 80)
			secparam.ifcbits = 1024;
		else if (secparam.symbits <= 112)
			secparam.ifcbits = 2048;
		else if (secparam.symbits <= 128)
			secparam.ifcbits = 3072;
		else if (secparam.symbits <= 192)
			secparam.ifcbits = 7680;
		else if (secparam.symbits <= 256)
			secparam.ifcbits = 15360;
		else
			secparam = LT;
	}
